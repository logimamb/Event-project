...
import { Event, ShareEventData } from '@/types'
import { shareEvent } from '@/app/actions/events'
...
export function ShareEventDialog({ event }: ShareEventDialogProps) {
  ...
  const shareableUrl = generateShareableUrl('event', event.id, event.shareableSlug)
  ...
  const handleShare = async (type: ShareEventData['type']) => {
    setIsLoading(true)
    try {
      ...
      await shareEvent(event.id, shareData)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle id="share-dialog-title">{t('shareEvent')}</DialogTitle>
